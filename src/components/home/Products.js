      ...
      {productData.map((item) => (
        ...
        <div className='w-full h-auto flex items-center justify-center relative group'>
          ...
          <div className='flex items-center justify-between'>
            <h2 className='font-titleFont tracking-wide text-lg text-amazon_blue font-medium'>{item.title}</h2>
            <p>INR {item.price}</p>
          </div>
          <div>
            <p className="text-sm">{item.description.substring(0,100)}...</p>
            <div className="text-yellow-500 flex"><span><StarIcon /><StarIcon /><StarIcon /><StarIcon /><StarIcon />
            </span></div>
          </div>
